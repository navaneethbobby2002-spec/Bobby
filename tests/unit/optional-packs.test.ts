...
test("packMemberInstalled probes installed pack trees with either path separator", () => {
  const dataDir = tmpDataDir();
  ...
  assert.equal(packMemberInstalled("onnxruntime-node/package.json", dataDir), true);
  assert.equal(packMemberInstalled(path.join("onnxruntime-node", "package.json"), dataDir), true);
  assert.equal(packMemberInstalled("@huggingface/transformers/package.json", dataDir), false);
  assert.equal(
    packMemberInstalled("onnxruntime-node/package.json", path.join(dataDir, "absent")),
    false
  );
  fs.rmSync(dataDir, { recursive: true, force: true });
});
